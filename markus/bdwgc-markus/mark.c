...
STATIC void GC_push_marked(struct hblk *h, hdr *hhdr)
{
    ...
    /* Some quick shortcuts: */
        ...
        if (GC_block_empty(hhdr)/* nothing marked */ || !IS_MY_MAPPED(hhdr)) return;
#   if !defined(GC_DISABLE_INCREMENTAL)
      GC_n_rescuing_pages++;
#   endif
    GC_objects_are_marked = TRUE;
...
STATIC struct hblk * GC_push_next_marked(struct hblk *h)
{
    ...
    if (EXPECT(IS_FORWARDING_ADDR_OR_NIL(hhdr) || HBLK_IS_FREE(hhdr) || !IS_MY_MAPPED(hhdr), FALSE)) {
  ...
  STATIC struct hblk * GC_push_next_marked_dirty(struct hblk *h)
  {
    ...
    for (;;) {
        if (EXPECT(IS_FORWARDING_ADDR_OR_NIL(hhdr)
                   || HBLK_IS_FREE(hhdr) || !IS_MY_MAPPED(hhdr), FALSE)) {
          h = GC_next_used_block(h);
          if (h == 0) return(0);
          hhdr = GC_find_header((ptr_t)h);
        } else {
