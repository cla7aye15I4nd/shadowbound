...
GC_INNER ptr_t GC_alloc_large(size_t lb, int k, unsigned flags)
{
    ...
    /* Do our share of marking work */
        if (GC_incremental && !GC_dont_gc && WALK_GC_FREEING_LIST)
...
/*if(FREEING_LIST && !ALWAYS_WALK && 0 == op && WALK_GC_FREEING_LIST && 0) {
ENTER_GC();
	 	GC_collect_or_expand(1, 0, 0);
	 	EXIT_GC();
            opp = &(kind -> ok_freelist[lg]);
            op = *opp;	    
}*/
...
{
//assert(0);
   // return GC_generic_malloc(lb,NORMAL);
    void *op;
    DCL_LOCK_STATE;
...
GC_API size_t malloc_usable_size(void* p) {
...
#   if defined(GC_LINUX_THREADS)
      ...
      /* mmapped thread stacks.  Make sure it is not collectible.       */
      {
        ...
        if (((word)caller >= (word)GC_libpthread_start
          ...
          return GC_generic_malloc(n * lb, NORMAL);
