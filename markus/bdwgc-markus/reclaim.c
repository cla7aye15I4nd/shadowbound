...
#  endif
        ...
        ptr_t base = (ptr_t)GC_base(leaked);
        int kind = HDR(base)->hb_obj_kind;
        if(kind == PTRFREE) return;
...
GC_INNER void GC_print_all_errors(void)
{
    ...
    for (i = 0; i < n_leaked; i++) {
        ...
        int kind = HDR(base)->hb_obj_kind;
        if(kind != PTRFREE) GC_print_heap_obj(p);
        size += kind == PTRFREE ? 0 : GC_size(base);
        //GC_free(p);
    }
    

 //   	    GC_err_printf("MarkUs: missing %lu bytes out of %lu\n",
 //                 size, ((u_int64_t)GC_heapsize-unmapped));
