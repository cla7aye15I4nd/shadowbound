...
word GC_gc_no = 0;
  ...
  GC_INNER GC_bool GC_incremental = TRUE; /* By default, stop the world. */
#endif
#else 
...
GC_INNER void GC_collect_a_little_inner(int n)
{
    ...
    GC_ASSERT(I_HOLD_LOCK());
    if(!WALK_GC_FREEING_LIST)return;
...
//printf("finishing collection\n");
#if FREEING_LIST
if(GC_is_full_gc || freeing_list_size > 0 || BIGSECTION)
GC_walk_freeing_list();

if(BLOCK_CULL)
GC_walk_freeing_link_hblker();
else
GC_walk_freeing_link(); 



#if 0
GC_mark_freeing_link(); 
GC_clear_freeing_link(); 
GC_free_freeing_link(); 
#endif



//return;
//printf("walked freeing list\n");
#endif
...
#   endif
    COND_DUMP;
    //MarkUs Note: Change this to if (TRUE) to pick up how many pointers the GC doesn't find but the program thinks are live (and may well be).
    if (GC_find_leak) {
                                      ...
                                      GC_bool retry)
{
    ...
    DISABLE_CANCEL(cancel_state);
    if (WALK_GC_FREEING_LIST  && !GC_incremental && !GC_dont_gc &&
...
GC_INNER ptr_t GC_allocobj(size_t gran, int kind)
{
    ...
    while (*flh == 0) {
      ENTER_GC();
#     ifndef GC_DISABLE_INCREMENTAL
        if (WALK_GC_FREEING_LIST && GC_incremental && GC_time_limit != GC_TIME_UNLIMITED) {
          /* True incremental mode, not just generational.      */
          /* Do our share of marking work.                      */
          GC_collect_a_little_inner(1);
        } 
#     endif
     /* if(*flh == 0 && FREEING_LIST && WALK_GC_FREEING_LIST  && GC_time_limit == GC_TIME_UNLIMITED) {
	 GC_collect_a_little_inner(1);
	}
      ...
      if (*flh == 0) {
        ...
        if (*flh == 0) {
          ENTER_GC();
          if (GC_incremental && 0 && GC_time_limit == GC_TIME_UNLIMITED
              && !tried_minor) {
            GC_collect_a_little_inner(1);
            tried_minor = TRUE;
          } else {
