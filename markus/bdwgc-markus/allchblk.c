...
//  if(GC_max_large_allocd_bytes > GC_heapsize) return 0;
                                       ...
                                       size_t bytes, int index)
{
    ...
    if(!IS_MY_MAPPED(hhdr) && UNMAP_PAGES) { 
    rest_hdr-> hb_flags |= WAS_UNMAPPED;
      //need to remap the first page of the rest
      word second_start = (word)rest & (~(GC_page_size-1));
      if((word)rest != second_start) {
        assert(0);
        ptr_t addr = mmap((ptr_t)second_start,(size_t)GC_page_size,PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
                           ...
                           hdr *nhdr, int index /* Index of free list */)
{
    ...
    /* Replace h with n on its freelist */
      ...
      if(!IS_MY_MAPPED(hhdr) && UNMAP_PAGES) { 
      	nhdr-> hb_flags |= WAS_UNMAPPED;
      	//need to remap the first page of the rest

      	word second_start = (word)n & (~(GC_page_size-1));
...
#               endif
    ...
    if (0 == hbp) return 0;
    if(!IS_MY_MAPPED(hhdr) && UNMAP_PAGES) flags |= WAS_UNMAPPED;	
...
#   endif
    ...
    GC_fail_count = 0;
    potential_remap(hhdr,hbp);
    //These allow us to reuse these links for our freeing list.
    hhdr->hb_next = NULL; 
    hhdr->hb_prev = NULL;
...
#   endif
    ...
    /* Coalesce with successor, if possible */
      if(0 != nexthdr && HBLK_IS_FREE(nexthdr) && IS_MAPPED(nexthdr)
         ...
         /* no overflow */ /*&& IS_MY_MAPPED(nexthdr) == mapped*/) {
      
       if(IS_MY_MAPPED(nexthdr) != mapped && UNMAP_PAGES) {
    	if(mapped) potential_unmap(hhdr,hbp); 
    	else potential_unmap(nexthdr,next); 
    	mapped = IS_MY_MAPPED(hhdr);
    	
              word mergepoint = (word)next & (~(GC_page_size-1));
      	      if(next != mergepoint) {
      	      assert(0);
                ptr_t addr = munmap((ptr_t)mergepoint,(size_t)GC_page_size);
     	pthread_mutex_lock(&unmap_lock);
     	unmapped += GC_page_size;
    ...
    /* Coalesce with predecessor, if possible. */
      if (0 != prev) {
        ...
        if (IS_MAPPED(prevhdr)
            && (signed_word)(hhdr -> hb_sz + prevhdr -> hb_sz) > 0  /*&& IS_MY_MAPPED(prevhdr) == mapped*/) {
        
    if(IS_MY_MAPPED(prevhdr) != mapped && UNMAP_PAGES) {
    	if(mapped) potential_unmap(hhdr,hbp); 
    	else potential_unmap(prevhdr,prev); 
    	mapped = IS_MY_MAPPED(hhdr);
    	
              word mergepoint = (word)hbp & (~(GC_page_size-1));
      	      if(hbp != mergepoint) {
      	        	      assert(0);
                ptr_t addr = munmap((ptr_t)mergepoint,(size_t)GC_page_size);
     	pthread_mutex_lock(&unmap_lock);
...
#         endif
    ...
    #if UNMAP_PAGES 
    //TODO!
       if(mapped && UNMAP_COST2(hhdr->hb_sz))potential_unmap(hhdr,hbp);
