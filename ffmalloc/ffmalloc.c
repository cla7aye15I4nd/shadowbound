...
#define GET_BIN(SIZE)                                                          \
  (SIZE <= 8 ? 0 : SIZE <= 304 ? BIN_COUNT - (SIZE >> 4) : PAGE_SIZE / SIZE)
#endif
...
#define FFAtomicExchangeAdvancePtr(DEST, VALUE)                                \
  DEST;                                                                        \
  DEST += VALUE
#define FFAtomicCompareExchangePtr(DEST, NEW, OLD)                             \
  (*DEST = *DEST == OLD ? NEW : *DEST) == NEW
...
#define FFAtomicSub(DEST, VALUE)                                               \
  InterlockedAdd64(&(LONG64)DEST, -(LONG64)(VALUE))
#define FFAtomicIncrement(DEST) InterlockedIncrement64(&DEST);
#define FFAtomicExchangeAdvancePtr(DEST, VALUE)                                \
  InterlockedExchangeAdd64(&(uintptr_t)DEST, VALUE)
#define FFAtomicCompareExchangePtr(DEST, NEW, OLD)                             \
  (InterlockedCompareExchangePointer(DEST, NEW, OLD) == OLD)
...
#define FFAtomicExchangeAdvancePtr(DEST, VALUE)                                \
  __sync_fetch_and_add(&(DEST), (byte *)VALUE)
#define FFAtomicCompareExchangePtr(DEST, NEW, OLD)                             \
  __sync_bool_compare_and_swap(DEST, OLD, NEW)
...
static void free_large_pointer(struct pagepool_t *pool, size_t index,
                               size_t size);
...
static inline LPVOID os_alloc(LPVOID startAddress, size_t size) {
  void *alloc = VirtualAlloc(startAddress, size, MEM_RESERVE | MEM_COMMIT,
                             PAGE_READWRITE);
  os_alloc_total += size;
  os_alloc_count++;
  return alloc;
...
static inline LPVOID os_alloc_highwater(size_t size) {
  ...
  if (result != NULL) {
    ...
    os_alloc_total += size;
    os_alloc_count++;
    while ((update = InterlockedCompareExchangePointer(&poolHighWater, newHigh,
                                                       oldHigh)) < newHigh) {
      oldHigh = update;
    }
  }
...
static inline void *os_alloc_highwater(size_t size) {
  ...
  while (result == NULL) {
    ...
    result = mmap(localHigh, size, PROT_READ | PROT_WRITE, flags, -1, 0);
...
// FFmalloc has several metadata structures that need to be dynamically
// allocated If we restricted usage to Windows or requiring use of the ff prefix
// on Linux then LocalAlloc or libc malloc respectively could be used. But, we'd
// like to allow non-prefixed usage via LD_PRELOAD on Linux (or static
// compilation so long as we're the first library). Therefore, we have a
// mini-allocator for metadata allocations. Because this should only be used
// internally, it does *not* implement the forward-only principal. Also note
// that the "free" equivalent requires a size parameter. This simplifies the
// amount of metadata for the metadata stored
...
static void *ffpoolmetadata_alloc(int isSmallPool) {
  ...
  if (metadatabins[isSmallPool] == NULL) {
    ...
    if (allocation + size > metadataEnd) {
      ...
      if (arenas[0]->profile.currentOSBytesMapped >
          arenas[0]->profile.maxOSBytesMapped) {
        arenas[0]->profile.maxOSBytesMapped =
...
struct pagepool_t *find_pool_for_ptr(const byte *ptr) {
  ...
  size_t leafIndex =
      ((uintptr_t)ptr >> (POOL_SIZE_BITS + LEAF_BITS)) & (LEAVES_PER_STEM - 1);

  // Find the correct leaf node
  struct radixleaf_t *leaf = poolTree.stems[stemIndex] != NULL
                                 ? poolTree.stems[stemIndex]->leaves[leafIndex]
                                 : NULL;
  if (leaf != NULL) {
    // Check if there is a pool that starts or ends in this leaf
    // that could possibly contain the given pointer
    struct pagepool_t *pool =
        leaf->poolStart[((uintptr_t)ptr >> POOL_SIZE_BITS) &
                        (POOLS_PER_LEAF - 1)];
    if (pool != NULL && ptr >= pool->start) {
      return pool;
    }
    pool = leaf->poolEnd[((uintptr_t)ptr >> POOL_SIZE_BITS) &
                         (POOLS_PER_LEAF - 1)];
...
void add_pool_to_tree(struct pagepool_t *pool) {
  ...
  size_t startStemIndex =
      (uintptr_t)pool->start >> (POOL_SIZE_BITS + LEAF_BITS + STEM_BITS);
  size_t startLeafIndex =
      ((uintptr_t)pool->start >> (POOL_SIZE_BITS + LEAF_BITS)) &
      (LEAVES_PER_STEM - 1);
  size_t startPoolIndex =
      ((uintptr_t)pool->start >> POOL_SIZE_BITS) & (POOLS_PER_LEAF - 1);
  size_t endStemIndex =
      (uintptr_t)pool->end >> (POOL_SIZE_BITS + LEAF_BITS + STEM_BITS);
  size_t endLeafIndex = ((uintptr_t)pool->end >> (POOL_SIZE_BITS + LEAF_BITS)) &
                        (LEAVES_PER_STEM - 1);
  size_t endPoolIndex =
      ((uintptr_t)pool->end >> POOL_SIZE_BITS) & (POOLS_PER_LEAF - 1);
  ...
  if (poolTree.stems[startStemIndex] == NULL) {
    poolTree.stems[startStemIndex] =
        (struct radixstem_t *)ffmetadata_alloc(sizeof(struct radixstem_t));
  }
  if (poolTree.stems[startStemIndex]->leaves[startLeafIndex] == NULL) {
    poolTree.stems[startStemIndex]->leaves[startLeafIndex] =
        (struct radixleaf_t *)ffmetadata_alloc(sizeof(struct radixleaf_t));
  }
  if (poolTree.stems[endStemIndex] == NULL) {
    poolTree.stems[endStemIndex] =
        (struct radixstem_t *)ffmetadata_alloc(sizeof(struct radixstem_t));
  }
  if (poolTree.stems[endStemIndex]->leaves[endLeafIndex] == NULL) {
    poolTree.stems[endStemIndex]->leaves[endLeafIndex] =
        (struct radixleaf_t *)ffmetadata_alloc(sizeof(struct radixleaf_t));
  }

  // Add the pool to the tree
  poolTree.stems[startStemIndex]
      ->leaves[startLeafIndex]
      ->poolStart[startPoolIndex] = pool;
  poolTree.stems[endStemIndex]->leaves[endLeafIndex]->poolEnd[endPoolIndex] =
      pool;
...
static unsigned int get_large_list_index() { return 0; }
...
void destroy_tcache(struct threadcache_t *tcache) {
  if (tcache->nextUnusedPage != NULL &&
      (tcache->nextUnusedPage < tcache->endUnusedPage)) {
    // While it would be better to return unused pages to the pool of origin,
    // that's more complicated than I want to handle right now. So, just give
    // them back to the OS
    os_decommit(tcache->nextUnusedPage->start,
                (tcache->endUnusedPage - tcache->nextUnusedPage) * PAGE_SIZE);
  }
}
...
static inline struct threadcache_t *get_threadcache(struct arena_t *arena) {
  ...
  if (tcache == NULL) {
    ...
    tcache =
        (struct threadcache_t *)ffmetadata_alloc(sizeof(struct threadcache_t));
...
static __attribute__((constructor)) void linux_mt_init() { initialize(); }
...
static inline struct threadcache_t *get_threadcache(struct arena_t *arena) {
  struct threadcache_t *tcache =
      (struct threadcache_t *)pthread_getspecific(arena->tlsIndex);
  if (tcache == NULL) {
    // No thread cache found so create one
    tcache =
        (struct threadcache_t *)ffmetadata_alloc(sizeof(struct threadcache_t));
...
static void assign_pages_to_tcache(struct threadcache_t *tcache) {
  ...
  // get distinct ranges
  nextFreePage = (byte *)FFAtomicExchangeAdvancePtr(
      pool->nextFreePage, (PAGES_PER_REFILL * PAGE_SIZE));
  ...
  while (nextFreePage + (PAGES_PER_REFILL * PAGE_SIZE) > pool->end) {
    ...
    if (pool == tcache->arena->smallPoolList->pool) {
      struct poollistnode_t *newListHeader =
          (struct poollistnode_t *)ffmetadata_alloc(
              sizeof(struct poollistnode_t));
      newListHeader->pool =
          (struct pagepool_t *)ffmetadata_alloc(sizeof(struct pagepool_t));
  ...
  for (size_t i = 0; i < PAGES_PER_REFILL; i++) {
    pool->tracking.pageMaps[nextFreePageMapIndex + i].start =
        nextFreePage + (i * PAGE_SIZE);
  }
...
static ffresult_t create_arena(struct arena_t *newArena) {
  ...
  newArena->smallPoolList =
      (struct poollistnode_t *)ffmetadata_alloc(sizeof(struct poollistnode_t));
  ...
  for (int i = 0; i < MAX_LARGE_LISTS; i++) {
    ...
    newArena->largePoolList[i] = (struct poollistnode_t *)ffmetadata_alloc(
        sizeof(struct poollistnode_t));
...
static int create_pagepool(struct pagepool_t *newPool) {
  // Get an initial range of virtual address space to hold the page maps and
  // bitmaps
  ...
  if (newPool->arena->profile.currentOSBytesMapped >
      newPool->arena->profile.maxOSBytesMapped) {
    // TODO: need thread safety
    newPool->arena->profile.maxOSBytesMapped =
        newPool->arena->profile.currentOSBytesMapped;
  }
#endif
...
#ifdef FF_PROFILE
  FFAtomicAdd(newPool->arena->profile.currentOSBytesMapped, POOL_SIZE);
  while (newPool->arena->profile.currentOSBytesMapped >
         newPool->arena->profile.maxOSBytesMapped) {
    // TODO: need thread safety
    newPool->arena->profile.maxOSBytesMapped =
        newPool->arena->profile.currentOSBytesMapped;
...
static inline int create_jumbopool(struct pagepool_t *newPool, size_t size) {
  ...
  if (newPool->arena->profile.currentBytesAllocated >
      newPool->arena->profile.maxBytesAllocated) {
    // TODO: thread safety
    newPool->arena->profile.maxBytesAllocated =
        newPool->arena->profile.currentBytesAllocated;
  }
  if (newPool->arena->profile.currentOSBytesMapped >
      newPool->arena->profile.maxOSBytesMapped) {
    // TODO: need thread safety
...
static void init_tcache(struct threadcache_t *tcache, struct arena_t *arena) {
  ...
  for (size_t b = 1; b <= (BIN_COUNT - BIN_INFLECTION); b++) {
    ...
    tcache->bins[BIN_COUNT - b].allocCount =
        tcache->bins[BIN_COUNT - b].maxAlloc;
...
static void initialize() {
  ...
  metadataPool =
      (byte *)mmap(NULL, 1024UL * 1048576UL, PROT_NONE,
...
#else
  metadataPool = (byte *)VirtualAlloc(NULL, 1024ULL * 1048576ULL, MEM_RESERVE,
                                      PAGE_NOACCESS);
...
static void destroy_pool(struct pagepool_t *pool) {
  ...
  if (pool->nextFreeIndex == SIZE_MAX) {
    ...
    byte *lastPage =
        pool->nextFreePage < pool->end ? pool->nextFreePage : pool->end;
    for (size_t i = 0; i < (lastPage - pool->start) / PAGE_SIZE; i++) {
      ...
      if (allocSize > 0 && allocSize < 64) {
        size_t bitmapCount = ((PAGE_SIZE / allocSize) & SIXTYTHREE64) != 0
                                 ? ((PAGE_SIZE / allocSize) >> 6) + 1
                                 : ((PAGE_SIZE / allocSize) >> 6);
        ffmetadata_free(pool->tracking.pageMaps[i].bitmap.array,
                        bitmapCount * 8);
      }
    }
...
static int64_t find_small_ptr(const byte *ptr, const struct pagepool_t *pool,
                              struct pagemap_t **pageMap) {
  // Find the location in the metadata where the page for this pointer is
  // tracked
...
static size_t find_large_ptr(const byte *ptr, struct pagepool_t *pool,
  ...
  while (left != right) {
    if (pool->tracking.allocations[current] == (uintptr_t)ptr) {
      ...
      *metadataIndex = current;
      return ((pool->tracking.allocations[current + 1] & ~SEVEN64) -
              pool->tracking.allocations[current]);
...
static void *ffmalloc_small(size_t size, struct arena_t *arena) {
  ...
  if (bin->maxAlloc <= 64) {
    ...
    FFAtomicOr(bin->page->bitmap.array[bin->allocCount >> 6],
               (ONE64 << (bin->allocCount & SIXTYTHREE64)));
  }
...
static inline void *ffmalloc_large_from_pool(size_t size, size_t alignment,
                                             struct pagepool_t *pool) {
  ...
  // benefit to doing so
#ifdef FF_PROFILE
  FFAtomicAdd(pool->arena->profile.totalBytesAllocated,
              size + ((byte *)alignedNext - pool->nextFreePage));
  FFAtomicAdd(pool->arena->profile.currentBytesAllocated,
              size + ((byte *)alignedNext - pool->nextFreePage));
  if (pool->arena->profile.currentBytesAllocated >
      pool->arena->profile.maxBytesAllocated) {
    // TODO: need thread safe update here
    pool->arena->profile.maxBytesAllocated =
        pool->arena->profile.currentBytesAllocated;
  }
#endif
  ...
  pool->tracking.allocations[++pool->nextFreeIndex] =
      (uintptr_t)pool->nextFreePage;
...
// if neccessary
static void *ffmalloc_large(size_t size, size_t alignment,
                            struct arena_t *arena) {
...
static void free_page(struct pagepool_t *pool, struct pagemap_t *pageMap) {
  ...
  if ((endAddress - startAddress >=
       (ptrdiff_t)(PAGE_SIZE * MIN_PAGES_TO_FREE)) ||
      (leftIsFreed != 0 && rightIsFreed != 0)) {
...
#ifdef FF_PROFILE
    FFAtomicSub(pool->arena->profile.currentOSBytesMapped,
                endAddress - startAddress);
#endif

    // Mark all of the pages as returned to the OS
    for (currentPage = leftmostPage; currentPage <= rightmostPage;
         currentPage++) {
      currentPage->allocSize |= 2UL;
    }

    // Update the "in use" pointers which measure the earliest and latest
    // address not yet freed in the pool. When those meet or cross then the
    // whole pool is completely freed and its metadata data can be freed
    if (startAddress <= pool->startInUse) {
      for (currentPage = rightmostPage;
           ((currentPage->allocSize & TWO64) != 0) &&
           (currentPage->start < pool->endInUse) &&
           (currentPage->start + PAGE_SIZE < pool->end);
           currentPage++) {
      }
    ...
    if (endAddress >= pool->endInUse) {
      for (currentPage = leftmostPage;
           ((currentPage->allocSize & TWO64) != 0) &&
           currentPage->start >= pool->startInUse &&
           (currentPage->start > pool->start);
           currentPage--) {
        if (currentPage->start > pool->start) {
    ...
    if (pool->startInUse >= pool->endInUse) {
      // All space in the pool has been allocated and subsequently freed.
      // Destroy this pool to free up metadata resources
      FFLeaveCriticalSection(&pool->poolLock);
      destroy_pool(pool);
      return;
    }
...
static inline void free_small_ptr(struct pagepool_t *pool,
                                  struct pagemap_t *pageMap, size_t index) {
#ifdef FF_PROFILE
  FFAtomicSub(pool->arena->profile.currentBytesAllocated,
...
static void free_large_pointer(struct pagepool_t *pool, size_t index,
...
#endif
  // Start searching for the start of the contiguous free region. The search
  // ends when the beginning of the list is reached, an in use block is found,
  // or a block that has already at least partially been unmapped
  firstFreeIndex = index;
  while ((firstFreeIndex > 0) &&
         ((pool->tracking.allocations[firstFreeIndex - 1] & THREE64) == 1)) {
    firstFreeIndex--;
  }
  ...
  // This is potentially the location to start unmapping from. However, it might
  // need to be adjusted forward or backwards depending on what the previous
  // block was marked as
  uintptr_t startFreeAddr =
      (pool->tracking.allocations[firstFreeIndex] & ~THREE64);
  if ((startFreeAddr & (PAGE_SIZE - 1)) != 0) {
    if ((pool->tracking.allocations[firstFreeIndex - 1] & TWO64) != 0) {
      ...
      startFreeAddr = ((startFreeAddr + PAGE_SIZE) & ~(PAGE_SIZE - 1));
             ...
             (pool->tracking.allocations[firstFreeIndex - 1] & TWO64)) {
    leftIsFreed++;
  }

  // Now start searching for the end of the contiguous free region. As before,
  // stop when the end of the list is reached, an allocated block is reached, or
  // a partially unmapped allocation is found
  lastFreeIndex = index;
  while ((lastFreeIndex < pool->nextFreeIndex) &&
         ((pool->tracking.allocations[lastFreeIndex + 1] & THREE64) == 1)) {
    lastFreeIndex++;
  }

  // Potentially the end of the region to unmap. If it doesn't fall on a page
  // boundary it will need to be adjusted forward or backwards
  uintptr_t endFreeAddr =
      (pool->tracking.allocations[lastFreeIndex + 1] & ~SEVEN64);
  if (endFreeAddr == 0) {
  ...
  if ((endFreeAddr & (PAGE_SIZE - 1)) != 0) {
    ...
    if ((byte *)endFreeAddr >= pool->end ||
        (pool->tracking.allocations[lastFreeIndex + 1] & TWO64)) {
      rightIsFreed++;
    }
  }
  ...
  if ((byte *)startFreeAddr <= pool->startInUse) {
    if ((byte *)endFreeAddr < pool->end) {
      ...
      while (contFreeIndex < pool->nextFreeIndex &&
             ((pool->tracking.allocations[contFreeIndex + 1] & TWO64) != 0)) {
        contFreeIndex++;
      }
      pool->startInUse =
          (byte *)(pool->tracking.allocations[contFreeIndex + 1] & ~SEVEN64);
    } else {
      pool->startInUse = pool->end;
    }
  }

  // The whole pool is now empty, destroy it (which will also finish freeing up
  // the indicated allocation)
  if ((pool->startInUse >= pool->endInUse) &&
      (pool->tracking.allocations[pool->nextFreeIndex] >=
       (uintptr_t)pool->end + FOUR64)) {
    FFLeaveCriticalSection(&pool->poolLock);
    destroy_pool(pool);
#ifdef FF_PROFILE
    // Update the physical memory statistics
    FFAtomicSub(pool->arena->profile.currentOSBytesMapped,
                (size_t)(endFreeAddr - startFreeAddr));
#endif
  } else if (endFreeAddr > startFreeAddr) {
    // Set a minimum number of pages before actually returning them to the OS
    // On Linux, this helps reduce fragmentation which results in VMA bloat
    // which can wrek our ability to request new pages. Alternately, if this is
    // an island between two free regions, return it regardless of size so that
    // 1) it doesn't get orphaned and 2) eliminates a VMA on Linux
    if ((endFreeAddr - startFreeAddr >= (PAGE_SIZE * MIN_PAGES_TO_FREE)) ||
        (leftIsFreed != 0 && rightIsFreed != 0)) {
      if (os_decommit((void *)startFreeAddr, endFreeAddr - startFreeAddr) ==
        ...
        if (errno == ENOMEM) {
          ...
          // being cleaned up, that way a future fffree can try again and
        ...
        fprintf(stderr, "Large pool decommit fail: %d, %p size: %ld\n", errno,
                (void *)startFreeAddr, endFreeAddr - startFreeAddr);
#else
        fprintf(stderr, "Large pool decommit fail: %d, %p size: %lld\n",
                GetLastError(), (void *)startFreeAddr,
                endFreeAddr - startFreeAddr);
#endif
        fflush(stderr);
        abort();
      }
...
void *ffrealloc(void *ptr, size_t size) {
  ...
  if (pool->nextFreeIndex < SIZE_MAX - 1) {
    ...
    if (size < POOL_SIZE) {
      ...
      if (index == pool->nextFreeIndex - 1 &&
          (pool->nextFreePage + additionalSize <= pool->end)) {
#ifdef FF_PROFILE
...
static inline void *ffmemalign_internal(size_t alignment, size_t size) {
  ...
  if (size <= HALF_PAGE && alignment <= HALF_PAGE) {
    if (size <= alignment) {
      ...
      return ffmalloc_small(ONE64 << (64 - FFCOUNTLEADINGZEROS64(size - 1)),
                            arenas[0]);
    }
  }
...
void *valloc(size_t size) { return ffmemalign(PAGE_SIZE, size); }
...
ffresult_t ffcreate_arena(ffarena_t *newArenaKey) {
  ...
  struct arena_t *newArena =
      (struct arena_t *)ffmetadata_alloc(sizeof(struct arena_t));
        ...
        if (leaf != NULL) {
          for (size_t l3 = 0; l3 < POOLS_PER_LEAF; l3++) {
            if (leaf->poolStart[l3] != NULL) {
              ...
              if (pool->nextFreeIndex == SIZE_MAX) {
                byte *lastFreePage = pool->end < pool->nextFreePage
                                         ? pool->end
                                         : pool->nextFreePage;
                for (size_t x = 0; x < (lastFreePage - pool->start) / PAGE_SIZE;
                     x++) {
                  if ((pool->tracking.pageMaps[x].allocSize & THREE64) == 3) {
                    released++;
                  } else if ((pool->tracking.pageMaps[x].allocSize & THREE64) ==
                             1) {
                    pending++;
                ...
                // {
                printf("Small pool addr: %p with %ld pages unassigned, ",
                       pool->start, unassigned);
 ...
 */
size_t ffget_pool_count() { return poolCount; }

#ifdef FF_PROFILE
void ffprint_stats_wrapper(void) { ffprint_statistics(stderr); }
...
static void print_current_usage() {
  // Should print: OS mem, current physical bytes, current alloc bytes, free
  // bytes need release free bytes awaiting assign
  ...
  if (usagePrintFile != NULL &&
      (arenas[0]->profile.mallocCount % usagePrintInterval == 0)) {
    for (size_t l1 = 0; l1 < STEM_COUNT; l1++) {
      if (poolTree.stems[l1] != NULL) {
        for (size_t l2 = 0; l2 < LEAVES_PER_STEM; l2++) {
          struct radixleaf_t *leaf = poolTree.stems[l1]->leaves[l2];
            ...
            for (size_t l3 = 0; l3 < POOLS_PER_LEAF; l3++) {
              if (leaf->poolStart[l3] != NULL) {
                ...
                if (pool->nextFreeIndex == SIZE_MAX) {
                  ...
                  smallPoolCount++;
                  poolMetadata +=
                      POOL_SIZE / PAGE_SIZE * sizeof(struct pagemap_t);
                  byte *lastFreePage = pool->end < pool->nextFreePage
                                           ? pool->end
                                           : pool->nextFreePage;
                  for (size_t x = 0;
                       x < (lastFreePage - pool->start) / PAGE_SIZE; x++) {
                    if ((pool->tracking.pageMaps[x].allocSize & THREE64) == 3) {
                      releasedPages++;
                    } else if ((pool->tracking.pageMaps[x].allocSize &
                                THREE64) == 1) {
                      pendingReleasePages++;
                    } else if (pool->tracking.pageMaps[x].allocSize == 0) {
                      tcachePages++;
    ...
    fprintf(usagePrintFile, fmtString, // peakOSReported,
            arenas[0]->profile.mallocCount, arenas[0]->profile.reallocCount,
            currentOSReported, arenas[0]->profile.currentOSBytesMapped,
            arenas[0]->profile.currentBytesAllocated, poolMetadata,
            smallPageWaste, smallFreeOnInUsePage,
            pendingReleasePages * PAGE_SIZE, pendingReleaseLargeBytes,
            (unassignedPages + tcachePages) * PAGE_SIZE, unassignedLargeBytes,
            largePoolAssigned, smallPoolCount, largePoolCount, jumboPoolCount,
            numEmptyLargePool);
  }
}
#endif
