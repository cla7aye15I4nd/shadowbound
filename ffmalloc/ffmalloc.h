...
#ifdef FF_WRAP_MMAP
void *ffmmap(void *addr, size_t length, int prot, int flags, int fd,
             off_t offset);
...
FFMALLOC_API ffresult_t ffmalloc_arena(ffarena_t arenaKey, void **ptr,
                                       size_t size);
...
FFMALLOC_API ffresult_t ffget_arena_statistics(ffprofile_t *profileDestination,
                                               ffarena_t arenaKey);

// Gets combined usage statistics for all arenas active or destroyed plus the
// default allocation arena.
// *** Not implemented yet ***
// FFMALLOC_API ffresult_t ffget_global_statistics(ffprofile_t*
// profileDestination);
...
FFMALLOC_API void ffprint_usage_on_interval(FILE *const dest,
